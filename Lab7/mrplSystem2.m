classdef mrplSystem
    ...
    methods
        ...
        function executeTrajectory(obj, iteration)
                ...
                %tStart = tic;
                tcurr = 0;
                x = obj.robot.encoders.LatestMessage.Vector.X;%obj.startPose(1);%
                y = obj.robot.encoders.LatestMessage.Vector.Y;%obj.startPose(2);%
                s = 0;
                ...
                while (s <= parms(3)) %t|| (errorth) > 0.02) %tcurr < lastT+tDelay)%
                    if (firstLoop) 
                        firstLoop = false;
                        tStart = tic;
                        tcurr = 0;
                    ...
                    senseX = getX(obj.estRobot);
                    senseY = obj.estRobot.y;
                    obj.sensedX = getX(obj.estRobot);
                    obj.sensedY = obj.estRobot.y;
                    %fprintf("sensed X %d\n", obj.sensedX);
                    %fprintf("sensed Y %d\n", obj.sensedY);
                    ds = ((newx - x)+(newy-y))/2;%sqrt((senseX-x)^2 + (senseY-y)^2);
                    s = s + ds;
                    t = trajectory.getTimeAtDist(s) + 0.005;
                    %fprintf("got time at dist   t = %d\n", t);
                    x = newx; %senseX; y = senseY;
                    y = newy;
